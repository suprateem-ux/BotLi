    ...
    steps:
      ...
      - name: Install dependencies & prepare
        run: |
          ...
          chmod +x ./engines/reckless-linux-avx2 ./engines/supraengine

      - name: Run Bot
        run: |
          echo "Starting bot..."
          python3 -u user_interface.py "matchmaking" "tournament Kqul9pAe" &
          PID=$!
      ...
      - name: 🔁 Self-Restart
        if: always()
        run: gh workflow run "Run botlilichess"
        env:
